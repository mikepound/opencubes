...
#[inline]
pub fn rot_matrix_points(map: &CubeMapPos, shape: &Dim, count: usize, x_col: MatrixCol, y_col: MatrixCol, z_col: MatrixCol, pmin: u16) -> CubeMapPos {
...
fn xy_rots_points(map: &CubeMapPos, shape: &Dim, count: usize, res: &mut CubeMapPos) {
    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::YN, MatrixCol::XN, MatrixCol::ZN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::YP, MatrixCol::XP, MatrixCol::ZN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::YP, MatrixCol::XN, MatrixCol::ZP, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::YN, MatrixCol::XP, MatrixCol::ZP, res.cubes[0]));
}
...
fn yz_rots_points(map: &CubeMapPos, shape: &Dim, count: usize, res: &mut CubeMapPos) {
    ...
    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::XN, MatrixCol::ZP, MatrixCol::YP, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::XN, MatrixCol::ZN, MatrixCol::YN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::XP, MatrixCol::ZP, MatrixCol::YN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::XP, MatrixCol::ZN, MatrixCol::YP, res.cubes[0]));
}
...
fn xyz_rots_points(map: &CubeMapPos, shape: &Dim, count: usize, res: &mut CubeMapPos) {
    //xz
    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::ZP, MatrixCol::YP, MatrixCol::XN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::ZN, MatrixCol::YN, MatrixCol::XN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::ZN, MatrixCol::YP, MatrixCol::XP, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::ZP, MatrixCol::YN, MatrixCol::XP, res.cubes[0]));
    
    //xyz
    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::ZP, MatrixCol::XN, MatrixCol::YN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::YP, MatrixCol::ZP, MatrixCol::XP, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::YN, MatrixCol::ZN, MatrixCol::XP, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::ZN, MatrixCol::XP, MatrixCol::YN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::YP, MatrixCol::ZN, MatrixCol::XN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::YN, MatrixCol::ZP, MatrixCol::XN, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::ZN, MatrixCol::XN, MatrixCol::YP, res.cubes[0]));

    *res = min(*res, rot_matrix_points(map, shape, count,
        MatrixCol::ZP, MatrixCol::XP, MatrixCol::YP, res.cubes[0]));
}
...
pub fn to_min_rot_points(map: &CubeMapPos, shape: &Dim, count: usize) -> CubeMapPos {
    ...
    res = min(res, rot_matrix_points(map, shape, count,
         MatrixCol::XP, MatrixCol::YN, MatrixCol::ZN, res.cubes[0]));

    res = min(res, rot_matrix_points(map, shape, count,
         MatrixCol::XN, MatrixCol::YP, MatrixCol::ZN, res.cubes[0]));

    res = min(res, rot_matrix_points(map, shape, count,
         MatrixCol::XN, MatrixCol::YN, MatrixCol::ZP, res.cubes[0]));
